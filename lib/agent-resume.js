...
const { getAgentAdapter, buildAgentPromptCommandTemplate } = require('./agent-adapters');
...
function resolveAgentResumeArg(commandText) {
    ...
    const adapter = getAgentAdapter(program);
    return adapter ? adapter.metadata().resumeArg : '';
}

function resolveAgentPromptCommandTemplate(commandText) {
    const normalizedCommand = String(commandText || '').trim();
    const program = resolveAgentProgram(commandText);
    return buildAgentPromptCommandTemplate(normalizedCommand, program);
}
