...
class PlaywrightPlugin {
    ...
    printMcpAdd(hostArg) {
        ...
        for (const sceneName of scenes) {
            ...
            this.writeStdout(`claude mcp add -t http -s user playwright-mcp-${sceneName} ${url}`);
        }
        this.writeStdout('');
        for (const sceneName of scenes) {
            const url = `http://${host}:${this.scenePort(sceneName)}/mcp`;
            this.writeStdout(`codex mcp add playwright-mcp-${sceneName} --url ${url}`);
        }
        this.writeStdout('');
        for (const sceneName of scenes) {
            const url = `http://${host}:${this.scenePort(sceneName)}/mcp`;
            this.writeStdout(`gemini mcp add -t http -s user playwright-mcp-${sceneName} ${url}`);
        }
