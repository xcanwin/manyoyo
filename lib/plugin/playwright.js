...
function convertManifestV2ToV3(extDir) {
    ...
    if (background && typeof background === 'object' && !Array.isArray(background)) {
        ...
        if (scripts.length > 0) {
            ...
            const swLines = [
                '// Auto-generated by manyoyo playwright ext-sync for MV3.',
...
function requestBuffer(url, timeoutMs, redirectsLeft = 5) {
    return new Promise((resolve, reject) => {
        const client = url.startsWith('https://') ? https : http;
        const req = client.get(url, {
            headers: {
                'user-agent': 'manyoyo-playwright-extension-sync/1.0'
            }
        }, (res) => {
            ...
            if ([301, 302, 303, 307, 308].includes(statusCode) && location && redirectsLeft > 0) {
                ...
                res.resume();
                resolve(requestBuffer(nextUrl, timeoutMs, redirectsLeft - 1));
                return;
            }
...
class PlaywrightPlugin {
    ...
    resolveConfig() {
        ...
        const defaultConfig = {
            ...
            extensionDir: path.join(pluginRootDir, 'extensions'),
            extensionTmpDir: path.join(pluginRootDir, 'tmp-crx'),
    ...
    async downloadFile(url, output, retries = 3, timeoutMs = 60_000) {
        ...
        for (let i = 1; i <= retries; i += 1) {
            try {
                // eslint-disable-next-line no-await-in-loop
                const data = await requestBuffer(url, timeoutMs);
    ...
    async syncExtensions(options = {}) {
        if (!this.ensureCommandAvailable('unzip')) {
            this.writeStderr('[ext-sync] failed: unzip command not found.');
            return 1;
        }

        const cleanTmp = options.cleanTmp === true;
        const prodversion = String(options.prodversion || this.config.extensionProdversion || '132.0.0.0').trim();
        const extDir = path.resolve(this.config.extensionDir);
        const tmpDir = path.resolve(this.config.extensionTmpDir);
    ...
    async run({ action, scene = 'host-headless', host = '', extensions = [], cleanTmp = false, prodversion = '' }) {
        if (action === 'ls') {
        ...
        if (action === 'ext-sync') {
            return await this.syncExtensions({ cleanTmp, prodversion });
        }
