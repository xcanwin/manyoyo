...
const { resolveSceneTargets } = require('./playwright-targets');
const { resolvePlaywrightConfig } = require('./playwright-config');
const {
...
} = require('./playwright-extension-paths');
...
class PlaywrightPlugin {
    ...
    resolveConfig() {
        return resolvePlaywrightConfig({
            ...
            resolveContainerRuntime: configuredRuntime => this.resolveContainerRuntime(configuredRuntime)
        });
    }
    ...
    resolveTargets(sceneName = 'all') {
        return resolveSceneTargets(sceneName, this.config);
    }

    resolveExtensionPaths(extensionArgs = []) {
        return resolveExtensionPaths(extensionArgs);
    }

    resolveNamedExtensionPaths(extensionNames = []) {
        return resolveNamedExtensionPaths(extensionNames, this.extensionDirPath());
    }

    resolveExtensionInputs(options = {}) {
        return resolveExtensionInputs({
            ...options,
            extensionRoot: this.extensionDirPath()
        });
    }

    buildExtensionLaunchArgs(extensionPaths) {
        return buildExtensionLaunchArgs(extensionPaths);
    }

    sanitizeExtensionMountName(value) {
        return sanitizeExtensionMountName(value);
    }

    buildContainerExtensionMounts(extensionPaths = []) {
        return buildContainerExtensionMounts(extensionPaths);
    }
