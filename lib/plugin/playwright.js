...
class PlaywrightPlugin {
    ...
    buildSceneLaunchArgs(extensionPaths = []) {
        ...
        if (this.config.disableWebRTC) {
            args.push(...DISABLE_WEBRTC_LAUNCH_ARGS);
        }
        return args;
    }
    ...
    ensureSceneConfig(sceneName, options = {}) {
        ...
        let payload = null;
        if (isCliScene(sceneName)) {
            payload = this.buildCliSceneConfig(sceneName, options);
        } else {
    ...
    async startContainer(sceneName, options = {}) {
        const runtime = this.ensureContainerRuntimeAvailable('up', sceneName);
        if (!runtime) {
            return 1;
        }
        ...
        try {
            const args = this.buildContainerComposeCommand(sceneName, composeFiles, ['up', '-d']);
    ...
    stopContainer(sceneName) {
        if (!this.ensureContainerRuntimeAvailable('down', sceneName)) {
            return 1;
        }
        ...
        try {
            this.runCmd(this.buildContainerComposeCommand(sceneName, [], ['down']), { env, check: true });
        } catch (error) {
            return error.returncode || 1;
        }
    ...
    statusContainer(sceneName) {
        const runtime = this.ensureContainerRuntimeAvailable('status', sceneName);
        if (!runtime) {
            return 1;
        }
    ...
    logsContainer(sceneName) {
        const runtime = this.ensureContainerRuntimeAvailable('logs', sceneName);
        if (!runtime) {
            return 1;
        }
    ...
    async startHost(sceneName, options = {}) {
        ...
        this.clearHostSceneRuntimeState(sceneName);

        let { port, managedPids, portReachable } = await this.getHostSceneRuntimeInfo(sceneName);
        ...
        if (await this.waitForPort(port)) {
            ...
            if (managedPids.length > 0) {
                this.writeScenePidFile(pidFile, managedPids[0]);
    ...
    async stopHost(sceneName) {
        const { pidFile, port, managedPids } = await this.getHostSceneRuntimeInfo(sceneName);
        this.clearHostSceneRuntimeState(sceneName);
        this.signalPids(managedPids);

        if (managedPids.length > 0) {
            await sleep(300);
        }

        const pidFromFile = this.readPidFilePid(pidFile);
        if (pidFromFile > 0) {
            this.signalPids([pidFromFile]);
            fs.rmSync(pidFile, { force: true });
        }
    ...
    async statusHost(sceneName) {
        const { pidFile, port, managedPids, portReachable } = await this.getHostSceneRuntimeInfo(sceneName);

        if (managedPids.length > 0 && portReachable) {
            this.writeStdout(`[status] ${sceneName} running (pid(s) ${managedPids.join(' ')})`);
            if (this.readPidFilePid(pidFile) <= 0) {
                this.writeScenePidFile(pidFile, managedPids[0]);
            }
            return 0;
        }
