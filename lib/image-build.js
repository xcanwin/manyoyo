...
const TOOL_MANIFEST_PATH = ['docker', 'tool-manifest.json'];

function loadToolManifest(rootDir) {
    const manifestPath = path.join(rootDir, ...TOOL_MANIFEST_PATH);
    let manifest;
    try {
        manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    } catch (error) {
        throw new Error(`无法读取镜像工具清单: ${manifestPath}`);
    }

    const values = [
        manifest && manifest.node && manifest.node.version,
        manifest && manifest.node && manifest.node.sha256 && manifest.node.sha256.x64,
        manifest && manifest.node && manifest.node.sha256 && manifest.node.sha256.arm64,
        manifest && manifest.jdtls && manifest.jdtls.url,
        manifest && manifest.jdtls && manifest.jdtls.fileName,
        manifest && manifest.jdtls && manifest.jdtls.sha256,
        manifest && manifest.gopls && manifest.gopls.version,
        manifest && manifest.npm && manifest.npm.version,
        manifest && manifest.languageServers && manifest.languageServers.pyright,
        manifest && manifest.languageServers && manifest.languageServers.typescriptLanguageServer,
        manifest && manifest.languageServers && manifest.languageServers.typescript,
        manifest && manifest.agentCli && manifest.agentCli.claude,
        manifest && manifest.agentCli && manifest.agentCli.codex,
        manifest && manifest.agentCli && manifest.agentCli.gemini,
        manifest && manifest.agentCli && manifest.agentCli.opencode
    ];
    if (!manifest || manifest.schemaVersion !== 1 || values.some(value => typeof value !== 'string' || !value.trim())) {
        throw new Error(`镜像工具清单格式无效: ${manifestPath}`);
    }
    return manifest;
}

function hasBuildArg(args, name) {
    return extractBuildArgValues(args).some(value => value.startsWith(`${name}=`));
}

function appendManifestBuildArgs(args, manifest) {
    const values = {
        NODE_VERSION: manifest.node.version,
        NODE_SHA256_X64: manifest.node.sha256.x64,
        NODE_SHA256_ARM64: manifest.node.sha256.arm64,
        JDTLS_URL: manifest.jdtls.url,
        JDTLS_FILE: manifest.jdtls.fileName,
        JDTLS_SHA256: manifest.jdtls.sha256,
        GOPLS_VERSION: manifest.gopls.version,
        NPM_VERSION: manifest.npm.version,
        PYRIGHT_VERSION: manifest.languageServers.pyright,
        TYPESCRIPT_LANGUAGE_SERVER_VERSION: manifest.languageServers.typescriptLanguageServer,
        TYPESCRIPT_VERSION: manifest.languageServers.typescript,
        CLAUDE_CODE_VERSION: manifest.agentCli.claude,
        CODEX_VERSION: manifest.agentCli.codex,
        GEMINI_VERSION: manifest.agentCli.gemini,
        OPENCODE_VERSION: manifest.agentCli.opencode
    };
    Object.entries(values).forEach(([name, value]) => {
        if (!hasBuildArg(args, name)) args.push('--build-arg', `${name}=${value}`);
    });
    return args;
}
...
function createBuildCacheContext(ctx) {
    ...
    return {
        ...
        nodeMirrors: [config.nodeMirror, 'https://nodejs.org/dist', 'https://mirrors.tencent.com/nodejs-release'].filter(Boolean),
...
function prepareNodeBuildCache(ctx, cache, archNode) {
    ...
    const nodeVersion = ctx.toolManifest.node.version;
    const expectedHash = ctx.toolManifest.node.sha256[archNode];
    const nodeKey = `node/${nodeVersion}/${archNode}`;
    const expectedFileName = `node-v${nodeVersion}-linux-${archNode}.tar.gz`;

    ensureDirectoryIfMissing(nodeCacheDir);
    const nodeTargetPath = path.join(nodeCacheDir, expectedFileName);
    const hasNodeCache = fs.existsSync(nodeTargetPath) && getFileSha256(nodeTargetPath) === expectedHash;
    ...
    for (const mirror of cache.nodeMirrors) {
        try {
            ...
            ctx.runCmd('curl', ['-fsSL', `${mirror}/v${nodeVersion}/${expectedFileName}`, '-o', nodeTargetPath], { stdio: 'inherit' });
...
function prepareJdtlsBuildCache(ctx, cache, imageTool, arch) {
    ...
    const jdtlsKey = `jdtls/${ctx.toolManifest.jdtls.fileName}`;
    ...
    try {
        ctx.runCmd('curl', ['-fsSL', ctx.toolManifest.jdtls.url, '-o', jdtlsPath], { stdio: 'inherit' });
        if (getFileSha256(jdtlsPath) !== ctx.toolManifest.jdtls.sha256) {
            fs.unlinkSync(jdtlsPath);
            throw new Error('JDT LSP SHA256 校验失败');
        }
...
function prepareGoplsBuildCache(ctx, cache, imageTool, arch) {
    ...
    try {
        ...
        ctx.runCmd('go', ['install', `golang.org/x/tools/gopls@${ctx.toolManifest.gopls.version}`], {
            stdio: 'inherit',
...
async function buildImage(options = {}) {
    ...
    const { RED, GREEN, YELLOW, BLUE, CYAN, NC } = ctx.colors;
    try {
        ctx.toolManifest = loadToolManifest(ctx.rootDir);
    } catch (error) {
        ctx.error(`${RED}错误: ${error.message}${NC}`);
        ctx.exit(1);
        return;
    }
    ...
    appendManifestBuildArgs(imageBuildArgs, ctx.toolManifest);
