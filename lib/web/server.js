...
function spawnWebTerminalProcess(ctx, containerName, cols, rows) {
    const terminalBootstrap = [
        ...
        '        done',
        '        case "$1" in',
        '            exec|review|login|logout|mcp|mcp-server|app-server|completion|sandbox|debug|apply|a|cloud|features|help)',
        '                command codex "$@"',
        '                ;;',
        '            *)',
        '                if [ "${MANYOYO_CODEX_COLOR:-0}" = "1" ]; then',
        '                    command codex --no-alt-screen "$@"',
        '                else',
        '                    NO_COLOR=1 CLICOLOR=0 CLICOLOR_FORCE=0 command codex --no-alt-screen "$@"',
        '                fi',
        '                ;;',
        '        esac',
        '    }',
