...
async function startWebServer(options) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        res.on('finish', () => {
            // 整段包 try/catch：这是 EventEmitter 回调，且注册在全局认证网关之前，
            // 这里抛任何异常都会冒泡成 uncaughtException，进而被 bin/manyoyo.js 的
            // 处理器 process.exit(1)——等于一条未认证请求就能打挂 serve。
            // 具体到 decodeURIComponent：畸形百分号编码（如 /api/sessions/%E0%A4%A）
            // 会抛 URIError。打点失败本身无关紧要，绝不能影响请求处理
            try {
                ...
                diagLog(ctx, 'http_request', {
                    ...
                    session: sessionMatch ? decodeSessionName(sessionMatch[1]) : undefined
                });
            } catch (e) {
                // 诊断打点失败静默忽略
            }
