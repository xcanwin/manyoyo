...
// claude 原生支持 --session-id/-r <uuid> 在 --print 模式下无需 TTY 即可续接会话，
// 优先于探测式（AGENT_RESUME_ARG_MAP）续接，避免历史文本被截断丢失结论。
const NATIVE_SESSION_RESUME_PROGRAMS = new Set(['claude']);
...
function buildClaudeAgentExecCommand(template, prompt, options = {}) {
    ...
    if (sessionId) {
        flagSpecs.push(options.resume
            ? { flag: `-r ${sessionId}`, pattern: /(?:^|\s)-r(?:\s|$)/ }
            : { flag: `--session-id ${sessionId}`, pattern: /(?:^|\s)--session-id(?:\s|$)/ });
    }
...
async function prepareWebAgentExecution(ctx, state, sessionRef, prompt) {
    ...
    if (useNativeSessionResume) {
        // 原生续接：已建立过 session id 就直接用 -r <id> 续接，不再探测；
        // 否则（首轮，或历史消息早于本机制引入、缺少 engineSessionId）用 --session-id <uuid> 建立新会话。
        if (hasPriorConversation && agentSession.engineSessionId) {
            sessionIdForRun = agentSession.engineSessionId;
            resumeAttempted = true;
            resumeSucceeded = true;
        } else {
            sessionIdForRun = crypto.randomUUID();
        }
    } else if (hasPriorConversation && agentMeta.resumeSupported && agentMeta.resumeCommand) {
        resumeAttempted = true;
...
function finalizeWebAgentExecution(state, sessionRef, agentSession, agentMeta, meta, result) {
    ...
    if (meta.engineSessionId) {
        patch.engineSessionId = meta.engineSessionId;
    }
