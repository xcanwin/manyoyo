...
const { ensureDefaultImage } = require('../image-pull');
const { compileContainerRun } = require('../runtime/driver');
...
const { toErrorPayload, toHttpAppError } = require('../core/app-error');
const { FileEventStore } = require('../core/event-store');
const { createStaticAssetResolver } = require('./static-assets');
const { createTerminalProcess } = require('./terminal-process');
const { handleDoctorRequest, handleMetaAgentsRequest } = require('./controllers/system');
const { handleWebConfigReadRequest, handleWebConfigWriteRequest } = require('./controllers/config');
const { handleDirectoryListRequest, handleDirectoryMkdirRequest } = require('./controllers/directories');
const {
...
const { bindTerminalWebSocket } = require('./controllers/terminal');
const {
...
const {
    getAgentAdapter,
    resolveYoloCommand: resolveYoloCommandFromMetadata,
    listAgentMetadata
...
const WEB_LOGIN_MAX_FAILURES = 5;
const WEB_LOGIN_FAILURE_WINDOW_MS = 60 * 1000;
...
function extractAgentMessageFromStructuredOutput(agentProgram, text) {
    if (agentProgram === 'codex') {
        ...
        return adapter ? adapter.extractFinalMessage(text) : '';
    }
...
function resolveContainerModeArgs(mode) {
    return resolveContainerMode(mode);
}

function resolveYoloCommand(yolo) {
    return resolveYoloCommandFromMetadata(yolo);
}
...
function buildCreateRuntime(ctx, state, payload) {
    ...
    const defaultCommand = buildDefaultCommand(shellPrefix, shell, shellSuffix) || '/bin/bash';
    const runSpec = {
        ...resolvedBase.runSpec,
        container: {
            ...resolvedBase.runSpec.container,
            name: containerName,
            mode: containerMode || 'common',
            modeArgs: contModeArgs.slice(),
            hostPath,
            containerPath,
            envFile: resolvedBase.envFile.slice(),
            env: { ...resolvedBase.env },
            volumes: resolvedBase.volumes.slice(),
            ports: resolvedBase.ports.slice(),
            extraArgs: Array.isArray(ctx.containerExtraArgs) ? ctx.containerExtraArgs.slice() : []
        },
        process: {
            ...resolvedBase.runSpec.process,
            prefix: shellPrefix || '',
            shell: shell || '',
            suffix: shellSuffix || ''
        }
...
async function ensureWebContainer(ctx, state, containerInput, messageSessionRef = null) {
    ...
    if (!ctx.containerExists(runtime.containerName)) {
        ensureDefaultImage({
            ...
            execute: (args, options) => ctx.dockerExecArgs(args, options),
            log: message => ctx.logger.info(message)
        });
        const args = compileContainerRun({
...
try {
    ...
    const revision = crypto.createHash('sha256').update(fs.readFileSync(realPath)).digest('hex');
...
function buildContainerFileWriteCommand(requestedPath, content, expectedRevision = '') {
    return buildWebContainerNodeCommand(`
...
const expectedRevision = ${JSON.stringify(String(expectedRevision || ''))};
...
try {
    ...
    const currentRevision = crypto.createHash('sha256').update(fs.readFileSync(realPath)).digest('hex');
    if (expectedRevision && expectedRevision !== currentRevision) {
        process.stdout.write(JSON.stringify({
            error: '文件已被外部修改，请重新加载后再保存',
            conflict: true,
            currentRevision
        }));
    } else {
        ...
        process.stdout.write(JSON.stringify({
            ...
            saved: true,
            size: savedStat.size,
            revision: crypto.createHash('sha256').update(fs.readFileSync(realPath)).digest('hex')
        }));
    }
} catch (e) {
    process.stdout.write(JSON.stringify({
...
function sendJson(res, statusCode, payload, extraHeaders = {}) {
    const body = payload && typeof payload === 'object' && (typeof payload.error === 'string' || payload.error instanceof Error)
        ? toErrorPayload(toHttpAppError(statusCode, payload))
        : payload;
...
function buildSessionSummary(ctx, state, containerMap, sessionRef) {
    ...
    const controlEvents = loadWebSessionControlEvents(state.webHistoryDir, sessionRef, agentSession.events);
    const eventProjection = projectSessionEvents(controlEvents);
    ...
    return {
        ...
        status: ['completed', 'interrupted', 'failed'].includes(eventProjection.status)
            ? eventProjection.status
            : (containerInfo.status || 'history'),
...
function sendFileAsset(res, filePath, cacheControl = 'no-store') {
...
function sendViteAsset(res, resolver, assetName) {
    sendFileAsset(res, resolver.resolveViteAsset(assetName), 'public, max-age=31536000, immutable');
}
...
async function handleWebAuthRoutes(req, res, pathname, ctx, state) {
    ...
    if (req.method === 'POST' && pathname === '/auth/login') {
        ...
        const remoteAddress = String(req.socket && req.socket.remoteAddress || 'unknown');
        const now = Date.now();
        const failures = (state.loginFailures.get(remoteAddress) || [])
            .filter(timestamp => now - timestamp < WEB_LOGIN_FAILURE_WINDOW_MS);
        if (failures.length >= WEB_LOGIN_MAX_FAILURES) {
            const retryAfter = Math.max(1, Math.ceil((WEB_LOGIN_FAILURE_WINDOW_MS - (now - failures[0])) / 1000));
            state.loginFailures.set(remoteAddress, failures);
            sendJson(res, 429, { error: '登录尝试过于频繁，请稍后重试' }, { 'Retry-After': String(retryAfter) });
            return true;
        }
        ...
        if (!(userOk && passOk)) {
            state.loginFailures.set(remoteAddress, [...failures, now]);
            sendJson(res, 401, { error: '用户名或密码错误' });
            return true;
        }

        state.loginFailures.delete(remoteAddress);
        const sessionId = createWebAuthSession(state, username);
        sendJson(
            res,
            200,
            { ok: true, username },
            { 'Set-Cookie': getWebAuthCookie(sessionId, isSecureWebRequest(req, ctx)) }
        );
        return true;
    }
    ...
    if (req.method === 'POST' && pathname === '/auth/logout') {
        ...
        sendJson(
            ...
            200,
            { ok: true },
            { 'Set-Cookie': getWebAuthClearCookie(isSecureWebRequest(req, ctx)) }
        );
        return true;
    }
...
function sendWebUnauthorized(req, res, pathname, ctx) {
    if (pathname.startsWith('/api/') || pathname.startsWith('/auth/')) {
        sendJson(res, 401, { error: 'UNAUTHORIZED' });
        return;
    }
    if (pathname === '/' || pathname === '') {
        sendRedirect(res, 302, '/auth/login', { 'Set-Cookie': getWebAuthClearCookie(isSecureWebRequest(req, ctx)) });
        return;
    }
    sendHtml(
        res,
...
async function handleWebApi(req, res, pathname, ctx, state) {
    ...
    const fileControllerDependencies = {
        sendJson,
        readJsonBody,
        getValidSessionRef,
        ensureWebContainer,
        execJsonCommandInWebContainer,
        buildContainerFileListCommand,
        buildContainerFileReadCommand,
        buildContainerFileWriteCommand,
        buildContainerFileMkdirCommand,
        inferFileLanguage,
        filePreviewMaxBytes: WEB_FILE_PREVIEW_MAX_BYTES,
        fileEditMaxBytes: WEB_FILE_EDIT_MAX_BYTES
    };
    const routes = [
        {
            method: 'GET',
            ...
            handler: async () => {
                await handleDirectoryListRequest(req, res, { sendJson, expandHomeAliasPath, fs, path, os });
            }
        },
        {
            method: 'POST',
            ...
            handler: async () => {
                await handleDirectoryMkdirRequest(req, res, { sendJson, readJsonBody, expandHomeAliasPath, fs, path });
            }
        },
        {
            method: 'GET',
            match: currentPath => currentPath === '/api/meta/agents' ? [] : null,
            handler: async () => {
                await handleMetaAgentsRequest(res, { sendJson, listAgentMetadata });
            }
        },
        {
            method: 'GET',
            match: currentPath => currentPath === '/api/doctor' ? [] : null,
            handler: async () => {
                await handleDoctorRequest(res, ctx, state, {
                    sendJson,
        ...
        },
        {
            ...
            handler: async () => {
                await handleWebConfigReadRequest(res, ctx, state, { sendJson, readWebConfigSnapshot, buildSafeWebConfigSnapshot });
            }
        },
        {
            method: 'PUT',
            match: currentPath => currentPath === '/api/config' ? [] : null,
            handler: async () => {
                await handleWebConfigWriteRequest(req, res, ctx, state, {
                    sendJson, readJsonBody, readWebConfigSnapshot, restoreWebConfigSecrets,
                    parseAndValidateConfigRaw, buildConfigDefaults, fs, path
                });
            }
        },
        {
            ...
            handler: async () => {
                await handleSessionsListRequest(req, res, ctx, state, {
                    sendJson,
        ...
        },
        {
            ...
            match: currentPath => currentPath === '/api/sessions' ? [] : null,
            handler: async () => {
                await handleSessionCreateRequest(req, res, ctx, state, {
                    sendJson,
                    readJsonBody,
                    buildCreateRuntime,
                    ensureWebContainer,
        ...
        },
        {
            ...
            handler: async match => {
                await handleSessionAgentCreateRequest(res, ctx, state, match[1], {
                    sendJson,
                    getValidSessionRef,
                    loadWebSessionHistory,
        ...
        {
            method: 'GET',
            match: currentPath => currentPath.match(/^\/api\/sessions\/([^/]+)\/events$/),
            handler: async match => {
                await handleSessionEventsRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    getValidSessionRef,
                    loadWebSessionHistory,
                    getWebAgentSession,
                    createEmptyWebAgentSession,
                    loadWebSessionControlEvents,
                    selectEventsAfterCursor,
                    buildWebSessionKey
                });
            }
        },
        {
            ...
            handler: async match => {
                await handleSessionMessagesRequest(res, ctx, state, match[1], { sendJson, getValidSessionRef, loadWebSessionHistory, getWebAgentSession, createEmptyWebAgentSession, buildWebSessionKey });
            }
        },
        {
            method: 'GET',
            ...
            handler: async match => {
                await handleSessionFileListRequest(req, res, ctx, state, match[1], fileControllerDependencies);
            }
        },
        {
            method: 'GET',
            ...
            handler: async match => {
                await handleSessionFileReadRequest(req, res, ctx, state, match[1], fileControllerDependencies);
            }
        },
        {
            method: 'PUT',
            ...
            handler: async match => {
                await handleSessionFileWriteRequest(req, res, ctx, state, match[1], fileControllerDependencies);
            }
        },
        {
            method: 'POST',
            ...
            handler: async match => {
                await handleSessionFileMkdirRequest(req, res, ctx, state, match[1], fileControllerDependencies);
            }
        },
        {
            method: 'GET',
            ...
            handler: async match => {
                await handleSessionAuditRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    getValidSessionRef,
                    buildWebSessionKey,
                    buildSessionAudit
        ...
        },
        {
            ...
            handler: async match => {
                await handleSessionDetailRequest(res, ctx, state, match[1], { sendJson, getValidSessionRef, buildWebSessionKey, buildSessionDetail, listWebManyoyoContainers });
            }
        },
        {
            method: 'PUT',
            ...
            handler: async match => {
                await handleSessionAgentTemplateRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    readJsonBody,
                    getValidSessionRef,
                    hasOwn,
        ...
        },
        {
            ...
            handler: async match => {
                await handleSessionRunRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    readJsonBody,
                    getValidSessionRef,
                    ensureWebContainer,
        ...
        },
        {
            ...
            handler: async match => {
                await handleSessionAgentRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    readJsonBody,
                    getValidSessionRef,
        ...
        },
        {
            ...
            handler: async match => {
                return handleSessionAgentStreamRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    readJsonBody,
                    getValidSessionRef,
                    appendWebSessionMessage,
                    ...
                    prepareWebAgentExecution,
                    removeWebSessionMessage,
                    patchWebSessionMessage,
                    createWebStreamEmitter,
                    buildWebSessionKey,
                    execAgentInWebContainerStream,
                    resolveWebStreamControlEventType,
                    finalizeWebAgentExecution
                });
            }
        },
        {
            ...
            handler: async match => {
                await handleSessionAgentStopRequest(req, res, ctx, state, match[1], {
                    sendJson,
                    getValidSessionRef,
                    stopWebAgentRun,
...
async function startWebServer(options) {
    ...
    const ctx = {
        ...
        createTerminalProcess: options.createTerminalProcess || createTerminalProcess,
    ...
    const state = {
        ...
        frontendDistDir: options.frontendDistDir || path.join(FRONTEND_DIR, 'dist'),
        staticAssets: createStaticAssetResolver(options.frontendDistDir || path.join(FRONTEND_DIR, 'dist')),
    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (!authSession) {
                sendWebUnauthorized(req, res, pathname, ctx);
                return;
            }

            if (req.method === 'GET' && pathname === '/') {
                sendFileAsset(res, path.join(state.frontendDistDir, 'index.html'));
                return;
            }

            const viteAssetMatch = pathname.match(/^\/app\/assets\/(.+)$/);
            if (req.method === 'GET' && viteAssetMatch) {
                sendViteAsset(res, state.staticAssets, `assets/${viteAssetMatch[1]}`);
                return;
            }
