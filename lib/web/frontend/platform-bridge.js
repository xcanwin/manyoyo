(function () {
    ...
    const platform = {
        kind: getNativeBridge()
            ? 'native-shell'
            : (getCapacitorBrowserPlugin() ? 'capacitor' : 'browser'),
        getStorageItem(key) {
        ...
        openExternal(url) {
            ...
            const capacitorBrowser = getCapacitorBrowserPlugin();
            if (capacitorBrowser) {
                capacitorBrowser.open({ url: text }).catch(function () {
                    openExternalFallback(text);
                });
                return true;
            }
