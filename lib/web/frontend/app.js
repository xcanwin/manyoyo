(function () {
    ...
    function createDisclosureIcon() {
        ...
        icon.setAttribute('class', 'disclosure-toggle-icon');
        icon.setAttribute('viewBox', '0 0 12 12');
        icon.setAttribute('aria-hidden', 'true');
        icon.setAttribute('focusable', 'false');

        const polyline = document.createElementNS('http://www.w3.org/2000/svg', 'polyline');
        polyline.setAttribute('points', '4 2.5 8 6 4 9.5');
    ...
    function createDisclosureButton(expanded, label) {
        ...
        const icon = createDisclosureIcon();
        button.appendChild(icon);
        return button;
    }

    function createTreePrefixSegment(hasLine) {
        const segment = document.createElement('span');
        segment.className = `tree-prefix-segment${hasLine ? ' has-line' : ''}`;
    ...
    function createTreePrefix(ancestorHasNext, isLastSibling, options) {
        ...
        (Array.isArray(ancestorHasNext) ? ancestorHasNext : []).forEach(function (hasNext) {
            prefix.appendChild(createTreePrefixSegment(hasNext));
        });
    ...
    function setDisclosureExpanded(control, expanded) {
        ...
        if (control.disclosureWrap) {
            control.disclosureWrap.classList.toggle('is-expanded', !!expanded);
        }
        if (control.root) {
            control.root.dataset.expanded = nextValue;
        }
    }
    ...
    function createTreeItem(options) {
        ...
        if (opts.trailingNode) {
            row.appendChild(opts.trailingNode);
        }
        ...
        const control = {
            ...
            disclosureWrap: prefixControl.disclosureWrap
        };
        ...
        return (Array.isArray(grouped) ? grouped : []).map(function (directoryGroup) {
            return {
                ...
                children: (Array.isArray(directoryGroup.containers) ? directoryGroup.containers : []).map(function (containerGroup) {
                    ...
                    addAgentBtn.className = 'secondary tree-node-action';
    ...
    function renderSessionTreeNodes(nodes, parentNode, ancestorHasNext, itemCounter) {
        (Array.isArray(nodes) ? nodes : []).forEach(function (node, index) {
            ...
            const item = createTreeItem({
                ...
                trailingNode: node.trailingNode
            });
