(function () {
    ...
    function ensureMarkdownRendererConfigured() {
        if (markdownRuntime.configured) {
            return markdownRuntime.available;
        }

        const markedApi = getMarkedApi();
        if (!markedApi || typeof markedApi.Renderer !== 'function' || typeof markedApi.use !== 'function') {
            markdownRuntime.configured = true;
            markdownRuntime.available = false;
            return false;
        }
        ...
        try {
            ...
            markdownRuntime.available = true;
        } catch (e) {
            markdownRuntime.available = false;
        }
        markdownRuntime.configured = true;
    ...
    function shouldRenderMarkdownMessage(msg) {
    ...
    function renderMarkdownContent(content) {
        ...
        if (!ensureMarkdownRendererConfigured()) {
            return '';
        }
        const markedApi = getMarkedApi();
        if (!markedApi) {
    ...
    function createMessageRow(msg, index) {
        ...
        if (shouldRenderMarkdownMessage(msg)) {
            const markdownNode = document.createElement('div');
            markdownNode.className = 'md-content';
            const renderedMarkdown = renderMarkdownContent(msg.content);
            if (renderedMarkdown) {
