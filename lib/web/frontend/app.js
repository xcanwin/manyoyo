(function () {
    ...
    const appBrandSub = document.getElementById('appBrandSub');
    ...
    const terminalMobileTools = document.getElementById('terminalMobileTools');
    const terminalMobileNote = document.getElementById('terminalMobileNote');
    const terminalMobileForm = document.getElementById('terminalMobileForm');
    const terminalMobileInput = document.getElementById('terminalMobileInput');
    ...
    const MOBILE_TERMINAL_LANDSCAPE_MEDIA = window.matchMedia('(max-width: 980px) and (orientation: landscape)');
    ...
    const WORKBENCH_STATE_STORAGE_KEY = 'manyoyo.web.workbenchState.v1';
    ...
    function getRuntimeBrand() {
        if (platform.kind === 'native-shell') {
            return {
                title: 'MANYOYO Desktop',
                subtitle: 'Desktop Container Workbench',
                pageTitle: 'MANYOYO Desktop'
            };
        }
        if (platform.kind === 'capacitor') {
            return {
                title: 'MANYOYO Mobile',
                subtitle: 'Remote Container Workbench',
                pageTitle: 'MANYOYO Mobile'
            };
        }
        return {
            title: 'MANYOYO Web',
            subtitle: 'Container Session Console',
            pageTitle: 'MANYOYO Web'
        };
    }
    ...
    function applyRuntimeBrand() {
        ...
        if (appBrand) {
            appBrand.textContent = brand.title;
        }
        if (appBrandSub) {
            appBrandSub.textContent = brand.subtitle;
        }
        if (terminalMobileNote) {
            terminalMobileNote.textContent = platform.kind === 'capacitor'
                ? '移动端可用下方输入条补充键盘输入，横屏时终端体验更完整。'
                : '触控设备可用下方输入条补充键盘输入，横屏时终端体验更完整。';
        }
    }
    ...
    function sendTerminalInput(data, options) {
        ...
        const opts = options && typeof options === 'object' ? options : {};
        if (opts.focusTerminal !== false && state.terminal.term) {
    ...
    function normalizeWorkbenchState(raw) {
        const source = raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {};
        const activeTab = Object.prototype.hasOwnProperty.call(VIEW_LABELS, source.activeTab)
            ? source.activeTab
            : 'activity';
        const mode = source.mode === 'command' ? 'command' : 'agent';
        return {
            active: typeof source.active === 'string' ? source.active.trim() : '',
            activeTab,
            mode
        };
    }
    ...
    function loadWorkbenchState() {
        try {
            const raw = platform.getStorageItem(WORKBENCH_STATE_STORAGE_KEY);
            if (!raw) {
                return;
            }
            const nextState = normalizeWorkbenchState(JSON.parse(raw));
            if (nextState.active) {
    ...
    function persistWorkbenchState() {
        try {
            platform.setStorageItem(WORKBENCH_STATE_STORAGE_KEY, JSON.stringify({
                active: state.active || '',
                activeTab: state.activeTab || 'activity',
                mode: state.mode === 'command' ? 'command' : 'agent'
            }));
        } catch (e) {
            // 忽略浏览器存储异常，避免影响主流程
        }
    ...
    function isMobileTerminalLandscape() {
        return isMobileLayout() && MOBILE_TERMINAL_LANDSCAPE_MEDIA.matches;
    }

    function ensureTerminalActiveState() {
        if (state.activeTab !== 'terminal' || !state.active) {
            return;
        }
        if (!ensureTerminalReady()) {
            return;
    ...
    function setActiveTab(tab) {
        ...
        syncUi();
        if (next === 'terminal' && state.active) {
            ensureTerminalActiveState();
            if (state.terminal.term) {
                state.terminal.term.focus();
            }
        }
    ...
    function syncUi() {
        ...
        document.body.classList.toggle('mobile-terminal-landscape', terminalTab && isMobileTerminalLandscape());
        ...
        if (terminalMobileTools) {
            terminalMobileTools.hidden = !(terminalTab && isMobileLayout());
        }
        if (filesPanel) {
    ...
    if (terminalMobileForm) {
        terminalMobileForm.addEventListener('submit', function (event) {
            event.preventDefault();
            const text = terminalMobileInput ? terminalMobileInput.value : '';
            if (!text) {
                return;
            }
            if (sendTerminalInput(text)) {
                terminalMobileInput.value = '';
            }
        });

        terminalMobileForm.addEventListener('click', function (event) {
            const button = event.target.closest('[data-term-mobile-action]');
            if (!button) {
                return;
            }
            const action = button.getAttribute('data-term-mobile-action') || '';
            const seq = TERM_KEY_SEQUENCES[action];
            if (!seq) {
                return;
            }
            sendTerminalInput(seq);
