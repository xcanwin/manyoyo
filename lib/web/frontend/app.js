(function () {
    ...
    function createTraceCodeBlock(label, text) {
        const wrap = document.createElement('div');
        wrap.className = 'trace-narrative-code-block';

        const head = document.createElement('div');
        head.className = 'trace-narrative-code-head';

        const labelNode = document.createElement('span');
        labelNode.className = 'trace-narrative-code-label';
        labelNode.textContent = label;
        head.appendChild(labelNode);

        const copyBtn = document.createElement('button');
        copyBtn.type = 'button';
        copyBtn.className = 'trace-narrative-copy-btn';
        ...
        const pre = document.createElement('pre');
        pre.className = 'trace-narrative-code';
    ...
    function createTraceEventCard(traceEvent) {
        const narrative = window.ManyoyoChatBehavior.buildTraceNarrative(traceEvent);
        const wrap = document.createElement('div');
        wrap.className = 'trace-narrative' + (narrative.tone === 'error' ? ' is-error' : '');

        const text = document.createElement('p');
        text.className = 'trace-narrative-text';
        text.textContent = narrative.sentence;
        wrap.appendChild(text);

        narrative.codeBlocks.forEach(function (block) {
            wrap.appendChild(createTraceCodeBlock(block.label, block.text));
        });
