(function () {
    ...
    const defaultPlatform = window.ManyoyoPlatform || {
    ...
    function create(options) {
        ...
        const platform = options && options.platform && typeof options.platform === 'object'
            ? options.platform
            : defaultPlatform;
        const onError = options && typeof options.onError === 'function'
            ? options.onError
            : function (message) { platform.alert(message); };
        ...
        async function loadFile(targetPath, entry) {
            ...
            if (requiresReadonlyConfirm) {
                const yes = platform.confirm(`文件较大（${formatBytes(fileSize)}），继续后将以只读方式全量预览，无法保存。是否继续？`);
                if (!yes) {
                    return;
                }
            }
