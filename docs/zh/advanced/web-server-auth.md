...
description: 介绍 MANYOYO --server 模式的认证网关、参数优先级、登录流程与对外监听安全建议。
---

# 网页服务认证与安全实践

本页聚焦 `manyoyo --server` 网页模式的认证行为与最小安全基线。
...
`--server` 模式采用全局认证网关，除登录相关放行路由外，其余页面和接口默认要求登录。
...
2. 确认 `--server-user` / `--server-pass` 与配置项一致
3. 使用 `manyoyo --show-config` 检查 `serverUser`/`serverPass` 的最终来源
