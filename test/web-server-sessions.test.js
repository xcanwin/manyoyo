...
describe('Web Server Session Clone/Duplicate/Cascade Delete', () => {
    ...
    test('clone-config creates a new container with the same runtime config but empty chat history', async () => {
        ...
        try {
            ...
            expect(runArgs).toEqual(expect.arrayContaining(['--name', 'source-a-copy', '--env', 'FOO=bar']));

            const cloneHistoryPath = path.join(webHistoryDir, 'source-a-copy.json');
    ...
    test('clone-config auto-generates -copy / -copy-2 suffix on repeated calls without a name', async () => {
        ...
        try {
            ...
            expect(first.json.name).toBe('source-b-copy');
            ...
            expect(second.json.name).toBe('source-b-copy-2');
        } finally {
            if (handle && typeof handle.close === 'function') {
                await handle.close();
            }
